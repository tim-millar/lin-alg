...
def equal(u,v):
    ...
    assert u.D == v.D
    return {d:getitem(u,d) for d in u.D} == {d:getitem(v,d) for d in u.D}

def add(u,v):
    "Returns the sum of the two vectors"
    assert u.D == v.D
    return Vec(u.D,{d:(getitem(u,d)+getitem(v,d)) for d in u.D if (getitem(u,d)+getitem(v,d)) != 0})
...
def scalar_mul(v, alpha):
    ...
    return Vec(v.D,{d:alpha*getitem(v,d) for d in v.D if getitem(v,d) != 0})
